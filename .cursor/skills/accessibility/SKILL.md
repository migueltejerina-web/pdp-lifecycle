...
Comprehensive accessibility guidelines based on WCAG 2.2, customized for the PDP Lifecycle Units stack.

## Vistral-Specific Guidance

### Radix UI — Built-in Accessibility

This project uses Radix UI primitives via `components/ui/`. Radix handles many WCAG requirements automatically:

| Radix Component | What it handles for you |
|-----------------|------------------------|
| `Dialog` | Focus trap, Escape to close, aria-labelledby, return focus |
| `DropdownMenu` | Arrow key navigation, typeahead, role=menu |
| `Select` | Listbox pattern, keyboard selection, aria-expanded |
| `Tabs` | Tab/arrow key navigation, role=tablist/tab/tabpanel |
| `Tooltip` | Delay, escape dismissal, accessible name |
| `AlertDialog` | Focus trap, required action before dismiss |
| `Checkbox / Switch` | Proper role, checked state, keyboard toggle |

**Rule:** Always use `components/ui/` Radix primitives instead of building custom interactive elements. If Radix has a primitive for it, use it — the accessibility is built in.

### i18n and Language

The app supports Spanish (primary) and English. Accessibility requirements:

- `<html lang="es">` must be set (or `en` when user switches language)
- The `useI18n()` hook manages language switching via `lib/i18n/`
- All visible text must come from translation keys (`t.section.key`), not hardcoded strings
- `aria-label` values must also use `useI18n()` for screen reader support in both languages

### Design Tokens

Use `--prophero-*` tokens from `app/prophero.css` for:

- **Color contrast:** The design system tokens are pre-validated for AA contrast ratios
- **Font sizes:** Use token scale (`--prophero-font-size-sm/md/lg`) — never go below 14px
- **Spacing:** Token spacing ensures touch targets meet the 24x24px minimum

### Forms (react-hook-form + Zod)

The project's form pattern (see `form-creation` skill) should include:

- `<label>` explicitly associated with every input via `htmlFor`
- Error messages linked to inputs via `aria-describedby`
- Required fields marked with `aria-required="true"`
- Form validation errors announced to screen readers (use `aria-live="polite"` on error container)

---

## WCAG 2.2 Reference (Generic)
