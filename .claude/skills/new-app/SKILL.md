...
**If any are missing:** ask the user in a single message:

> "To create the Supabase schema for the new app, I need three values from the **dev branch** of the Supply project.
>
> **Easiest way — 1Password:**
> Open 1Password and search for **"Vistral LAB AI Dashboard"** — all three values are stored there under the `dev` environment.
>
> **Alternative — Supabase Dashboard:**
> 👉 [supabase.com/dashboard](https://supabase.com/dashboard) → **Supply project** → **Settings → API**
> Make sure to select the **`dev` branch** (top-left branch switcher) before copying.
>
> Please paste:
> 1. **Project URL** (`NEXT_PUBLIC_SUPABASE_URL`) — looks like `https://xxxx.supabase.co`
> 2. **Anon / public key** (`NEXT_PUBLIC_SUPABASE_ANON_KEY`) — the `anon` key under "Project API keys"
> 3. **Service role key** (`SUPABASE_SERVICE_ROLE_KEY`) — the `service_role` key (keep this secret, never share it publicly)"
